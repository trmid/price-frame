...
export const getStakedPTP = async (wallet) => {
...
export const getPoolBalances = async (wallet) => {
    ...
    let promises = pools.map(poolID => (async () => {
        ...
        if (userInfoResults) {
            ...
            if (balance > 0) {
                let token = (await query(chain, masterChef, platypus.masterChefABI, 'poolInfo', [poolID])).lpToken;
                let underlyingToken = await query(chain, token, platypus.tokenABI, 'underlyingToken', []);
...
export const getFactoryPoolBalances = async (wallet) => {
    ...
    let promises = pools.map(poolID => (async () => {
        ...
        if (userInfoResults) {
            let balance = parseBN(userInfoResults[0]);
            if (balance > 0) {
                let token = (await query(chain, factoryChef, platypus.factoryChefABI, 'poolInfo', [poolID])).lpToken;
                let underlyingToken = await query(chain, token, platypus.tokenABI, 'underlyingToken', []);
